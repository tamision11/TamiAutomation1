...
Feature: QM - B2B - Automatic BH
  ...
  Rule: Therapy
    ...
    Scenario: Flow 130 - Anthem - Therapy
    Anthem allowed prefixes are set on anthem_allowed_prefixes DB table
  ...
  Rule: Couples therapy
    ...
    Scenario: Flow 130 - Anthem - Couples therapy
      And Click on secure your match button
      And Click on continue on coverage verification
      And Payment - Complete purchase using "visa" card for primary user
      And Create account for primary user with
      ...
      And Navigate to payment and plan URL
      Then Plan details for the first room are
        | plan name | Anthem BH Couples Messaging or Live |
      And Payment and Plan - Waiting to be matched text is displayed for the first room
  ...
  Rule: Psychiatry
    ...
    Scenario: Flow 130 - Anthem - Psychiatry
    ...
    select if (maximum_allowed_vouchers >= usage_count , "vouchers available", "no available vouchers") from commercial.access_code_definitions where access_code_type = 'voucher_premera_bh_psychiatry';
      ...
      And Navigate to payment and plan URL
      Then Plan details for the first room are
        | plan name | Anthem BH Psychiatry |
      And Payment and Plan - Waiting to be matched text is displayed for the first room